...
public class HTPIController : MonoBehaviour
{
    ...
    private void Setup(Object obj, EventArgs empty)
    {
	    ...
	    if (GameManager.PlayerData?.SelectedActions == null)
                return;
                	    
	    foreach (var acao in GameManager.PlayerData.SelectedActions)
	    {
		// evita duplicar
		if (_resolucoes.Values.Contains(acao))
		    continue;

		var demanda = _resolucoes.Keys.FirstOrDefault(d => _resolucoes[d] == null);
	...
	_resolucoes[_botaoDemanda.Demanda] = acao;    
        if (!GameManager.PlayerData.SelectedActions.Contains(acao))
        {
           GameManager.PlayerData.SelectedActions.Add(acao);
           GameManager.Save();
        }    
       _botaoDemanda.Select();
       ScrollHtpi.DemandList.GoDown();    
       if (_resolucoes.Count(x => x.Value != null) ==
        GameManager.GameData.Demandas.Count(x => x.dia == GameManager.PlayerData.Day))
       {
           Confirmation();
       }
    }
